...
> ⚠️ **容器能做串口、也能开多端口** —— 别被"容器做不到"误导（两条都实测过）：
> - **串口**：容器只是默认不带宿主串口设备。**Linux 宿主**加
>   `devices: ["/dev/ttyUSB0:/dev/ttyUSB0"]` 即可用（实测容器内成功打开并写入）；
>   **Windows 宿主**（Docker Desktop 走 WSL2）**传不进 COM 口** ⇒ 串口请改走裸机部署。
> - **多端口**：容器内绑**任意**端口都行（含 502，无特权端口限制），实测 502/503/504/505 四个从站同时可读写；
