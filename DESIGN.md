...
| ⚠️ 草稿归属 | 草稿按**窗口身份**（标签 + 区域 + 起址 + 数量，由 `registerWindowKey()` 生成）**分桶保存**。换窗口 ⇒ 落进不同的桶（不串值、不会写错区）；切回原窗口 ⇒ 仍是同一个桶，**草稿原样还在**。草稿本体不带区域，「同一标签内改区域」曾因此把离散输入区的草稿显示（并可提交）到别的区 —— 但**"不串"不能靠"清空"实现**，清空会把切回来的草稿一起清掉 |
